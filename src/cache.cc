...
Node::Node(unsigned short id, size_t nrules, bool prediction,
              ...
              Node* parent, size_t num_captured, double equivalent_minority, double proportion, double default_objective)
...
CacheTree::CacheTree(size_t nsamples, size_t nrules, double c, rule_t *rules,
    ...
    : root_(0), nsamples_(nsamples), nrules_(nrules), c_(c),
      ...
      opt_rulelist_({}), opt_predictions_({}), type_(type), wpa_(wpa), x_(x) {
...
Node* CacheTree::construct_node(unsigned short new_rule, size_t nrules, bool prediction,
                         ...
                         int len_prefix, double c, double equivalent_minority, double proportion, double default_objective) {
    ...
    if (strcmp(type_, "curious") == 0) {
        ...
        n = (Node*) (new CuriousNode(new_rule, nrules, prediction, default_prediction,
                                lower_bound, objective, curiosity, (CuriousNode*) parent, num_captured, equivalent_minority, proportion, default_objective));
    } else {
        n = (new Node(new_rule, nrules, prediction, default_prediction,
                                lower_bound, objective, parent, num_captured, equivalent_minority, proportion, default_objective));
    }
