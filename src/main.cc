...
int main(int argc, char *argv[]) {
    ...
    while ((ch = getopt(argc, argv, "bsLdewWc:p:v:n:r:f:a:o:t:R:B:i:x:")) != -1) {
        switch (ch) {
        case 'b':
            run_bfs = true;
            break;
    ...
    snprintf(froot, BUFSZ, "../logs/for-%s-%s%s-%s-%s-removed=%s-max_num_nodes=%d-c=%.7f-v=%s-f=%d-wpa=%d-falling=%d-t=%f-B=%f-R=%f-i=%d-x=%d",
            pch ? pch + 1 : "",
            run_bfs ? "bfs" : "",
            ...
            max_num_nodes, c, "0", freq, wpa, falling, ties, bound, random, max_iter, x);
    ...
    bbound(tree, max_num_nodes, q, p, falling, show_proportion, change_search_path, ties, random, bound, max_iter, x);
